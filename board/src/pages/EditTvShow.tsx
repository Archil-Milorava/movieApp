...
import { useGetSingleTvShow, useUpdateTvShow } from "../features/tvShows/useMoviesHook";

const EditTvShowPage = () => {
  const { id } = useParams();
  const [showModal, setShowModal] = useState(false);
  const [description, setDescription] = useState("");

  const { singleTvShow, isLoading, error } = useGetSingleTvShow(id!);
  const { performUpdate, isPending, error: submitError } = useUpdateTvShow();
    ...
    <div className="bg-[#EDE9E6] min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-3xl bg-white p-6 rounded-md shadow-md">
        ...
        <div className="flex flex-col sm:flex-row gap-6 items-center mb-6">
          <img
            src={singleTvShow?.posterPath}
            alt={singleTvShow?.title}
            className="w-40 h-60 object-cover rounded-md shadow"
          />
          <textarea
            value={description}
            onChange={(e) => setDescription(e.target.value)}
            placeholder="Edit TV show description..."
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white p-6 rounded-md shadow-md w-[90%] max-w-sm text-center">
            ...
            <p className="mb-6 text-sm text-gray-600">
              You are about to submit changes to this TV show.
            </p>
